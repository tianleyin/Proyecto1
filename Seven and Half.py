...
def generategame_id():
    ...
    miConexion = mysql.connector.connect(host='sieteymedio.mysql.database.azure.com', user='marcelogr04',passwd='Proyecto7', db='proyecto7')
    cur = miConexion.cursor()
    cur.execute(f'select game_id from game;')
    for cardgame_id in cur.fetchall():
        if cardgame_id==a:
            return generatecardgame_id()
    miConexion.close()
    return a

def capcelera_princi():
    s1 = (" _____                                               _   _   _         _   __ ")
    s2 = ("/  ___|                                             | | | | | |       | | / _|")
    s3 = ("\ `--.   ___ __   __  ___  _ __     __ _  _ __    __| | | |_| |  __ _ | || |_ ")
    s4 = ("  `--. \ / _ \\ \ / / / _ \| '_ \   / _` || '_ \  / _` | |  _  | / _` || ||  _|")
    s5 = ("/\__/ /|  __/ \ V / |  __/| | | | | (_| || | | || (_| | | | | || (_| || || |  ")
    s6 = ("\____/  \___|  \_/   \___||_| |_|  \__,_||_| |_| \__,_| \_| |_/ \__,_||_||_|  ")
    s7 =("")
    print(s1+ "\n"+ s2, "\n"+ s3, "\n"+ s4+ "\n"+ s5+ "\n"+ s6+"\n"+s7)
    print("-"*78)
def capcelera_players():
    s1 = (
        "  ___       _      _      __ _____  _                           ________                                      ")
    s2 = (
        " / _ \     | |    | |    / //  ___|| |                         / /| ___ \                                     ")
    s3 = (
        "/ /_\ \  __| |  __| |   / / \ `--. | |__    ___  __      __   / / | |_/ /  ___  _ __ ___    ___  __   __  ___ ")
    s4 = (
        "|  _  | / _` | / _` |  / /   `--. \| '_ \  / _ \ \ \ /\ / /  / /  |    /  / _ \| '_ ` _ \  / _ \ \ \ / / / _ ")
    s5 = (
        "| | | || (_| || (_| | / /   /\__/ /| | | || (_) | \ V  V /  / /   | |\ \ |  __/| | | | | || (_) | \ V / |  __/")
    s6 = (
        "\_| |_/ \__,_| \__,_|/_/    \____/ |_| |_| \___/   \_/\_/  /_/    \_| \_| \___||_| |_| |_| \___/   \_/   \___|")
    s7 = ("")
    print(s1 + "\n" + s2, "\n" + s3, "\n" + s4 + "\n" + s5 + "\n" + s6 + "\n" + s7)
    print("-" * 110)
def capcelerasettings():
    s1 = (" _____        _    _    _                    ")
    s2 = ("/  ___|      | |  | |  (_)                   ")
    s3 = ("\ `--.   ___ | |_ | |_  _  _ __    __ _  ___ ")
    s4 = (" `--. \ / _ \| __|| __|| || '_ \  / _` |/ __|")
    s5 = ("/\__/ /|  __/| |_ | |_ | || | | || (_| |\__ \ ")
    s6 = ("\____/  \___| \__| \__||_||_| |_| \__, ||___/")
    s7 = ("                                   __/ |     ")
    s8 = ("                                  |___/      ")

    print(s1+ "\n"+ s2, "\n"+ s3, "\n"+ s4+ "\n"+ s5+ "\n"+ s6+"\n"+s7+"\n"+s8)
    print("-"*110)
def capcelera_ranking():
    s1 = (" _____                _     _               ")
    s2 = ("| ___ \              | |   (_)              ")
    s3 = ("| |_/ /  __ _  _ __  | | __ _  _ __    __ _ ")
    s4 = ("|    /  / _` || '_ \ | |/ /| || '_ \  / _` |")
    s5 = ("| |\ \ | (_| || | | ||   < | || | | || (_| |")
    s6 = ("\_| \_| \__,_||_| |_||_|\_\|_||_| |_| \__, |")
    s7 = ("                                       __/ |")
    s8 = ("                                      |___/ ")
    print(s1+ "\n"+ s2, "\n"+ s3, "\n"+ s4+ "\n"+ s5+ "\n"+ s6+"\n"+s7+"\n"+s8)
    print("-"*110)
...
def introduce_players():
    miConexion = mysql.connector.connect(host='sieteymedio.mysql.database.azure.com', user='marcelogr04',passwd='Proyecto7', db='proyecto7')
    cur = miConexion.cursor()
    cur.execute("select player_id, player_name, player_risck, human from player")
    for player_id, player_name, player_risck, human in cur.fetchall():
        if human==1:
            players[player_id] = {"name": player_name, "human": True, "bank": False, "initialCard": "", "priority": 0, "type": player_risck, "bet": 4, "points": 20, "cards": [], "roundPoints": 0}
        else:
            players[player_id] = {"name": player_name, "human": False, "bank": False, "initialCard": "", "priority": 0,"type": player_risck, "bet": 4, "points": 20, "cards": [], "roundPoints": 0}
    miConexion.close()
introduce_players()
def menu_principal():
...
def play_game():
    ...
    if len(game)<2:
        ...
        if contin == "y":
            cardgame["cardgame_id"]=generatecardgame_id()
            cardgame["players"]=len(game)
            cardgame["start_hour"]=str(datetime.now())[0:19]
            cardgame["rounds"]=rounds[0]
            cardgame["deck_id"]=mazo[0]
            for i in game:
...
def priority_players():
    ...
    for i in game:
        ...
        player_game[gaame][i]=dict(cardgame_id=0,initial_card_id=0,starting_points=0,ending_points=0)
        player_game[gaame][i]["initial_card_id"]=a
        player_game[gaame][i]["starting_points"]=20
        player_game[gaame][i]["cardgame_id"]=cardgame["cardgame_id"]
    order_players()
    restart_cards()
    input("Press any key to continue:")
    bank_player()
...
def order_players():
    for pasada in range(
        ...
        for i in range(len(game) - 1 - pasada):
            if cartas[mazo[0]][players[game[i]]["cards"][0]]["value"] < cartas[mazo[0]][players[game[i + 1]]["cards"][0]][
                "value"]:
                game[i], game[i + 1] = game[i + 1], game[i]
            if cartas[mazo[0]][players[game[i]]["cards"][0]]["value"] == cartas[mazo[0]][players[game[i + 1]]["cards"][0]][
                "value"]:
                if cartas[mazo[0]][players[game[i]]["cards"][0]]["priority"] < cartas[mazo[0]][players[game[i + 1]]["cards"][0]][
                "priority"]:
                    game[i], game[i + 1] = game[i + 1], game[i]
    pass
def bank_player():
...
def rounds_game():
    ...
    player_game_round[count_rounds[0]]={}
...
def rounds_starts():
    ...
    for i in game[1:]:
        ...
        player_game_round[count_rounds[0]][i]=dict(is_bank=0,bet_points=players[i]["bet"],starting_round_points=players[i]["points"],cards_value=players[i]["roundPoints"],endind_round_points=0,cardgame_id=cardgame["cardgame_id"],game_id=gaame)
...
def results():
    player_game_round[count_rounds[0]][game[0]] = dict(is_bank=1, bet_points=0,starting_round_points=players[game[0]]["points"],cards_value=players[game[0]]["roundPoints"], endind_round_points=0,cardgame_id=cardgame["cardgame_id"],game_id=gaame)
...
def show_payments():
    player_game_round[count_rounds[0]][game[0]]["endind_round_points"]=players[game[0]]["points"]
    print("*"*43,"Results","*"*43)
    input("Press ENTER to see the results")
    for i in game[1:]:
        player_game_round[count_rounds[0]][i]["endind_round_points"]=players[i]["points"]
        print("-"*43,players[i]["name"],"-"*43)
...
def player_eliminated(i):
    ...
    players[i]["cards"].clear()
    game.remove(i)
    player_game[gaame][i]["ending_points:"]=0
    if len(game)==1:
        return Game_Over()
    else:
        pass
...
def Game_Over():
    ...
    print(s1, "\n", s2, "\n", s3, "\n", s4, "\n", s5, "\n", s6, "\n", s7)
    print("-"*95)
    if count_rounds[0]+1 == rounds[0] or count_rounds[0]+1 == 30:
        print("The maximum rounds have been reached")
        if len(game)>1:
            for i in game:
                player_game[gaame][i]["ending_points"]=players[i]["points"]
                for pasada in range(
                        len(game) - 1):
                    for i in range(len(game) - 1 - pasada):
                        if players[game[i]]["points"] < \
                            ...
                            game[i], game[i + 1] = game[i + 1], game[i]
    player_game[gaame][game[0]]["ending_points"]=players[game[0]]["points"]
    print("The winner is", players[game[0]]["name"])
    cardgame["end_hour"]=str(datetime.now())[0:19]
...
def save_player_db(i):
    ...
    bdri = players[i]["type"]
    if players[i]["human"] == False:
        cur.execute(f'insert into player (player_id,player_name,player_risck,human) values("{bdid}","{bdna}",{bdri},0);')
        miConexion.commit()
    else:
        cur.execute(f'insert into player (player_id,player_name,player_risck,human) values("{bdid}","{bdna}",{bdri},1);')
        miConexion.commit()
    miConexion.close()

def remove_player_bd(i):
    miConexion = mysql.connector.connect(host='sieteymedio.mysql.database.azure.com', user='marcelogr04',passwd='Proyecto7', db='proyecto7')
    cur = miConexion.cursor()
    cur.execute(f'delete from player where player_id="{i}";')
    miConexion.commit()
    miConexion.close()
def generatecardgame_id():
    a=random.randint(0,10000)
    miConexion = mysql.connector.connect(host='sieteymedio.mysql.database.azure.com', user='marcelogr04',passwd='Proyecto7', db='proyecto7')
    cur = miConexion.cursor()
    cur.execute(f'select cardgame_id from cardgame;')
    for cardgame_id in cur.fetchall():
        if cardgame_id==a:
            return generatecardgame_id()
    miConexion.close()
...
def implementcardgame():
    if cardgame["deck_id"]== "Baraja Española":
        cardgame["deck_id"]="ESP"
    else:
        cardgame["deck_id"]="POK"

    miConexion = mysql.connector.connect(host='sieteymedio.mysql.database.azure.com', user='marcelogr04',passwd='Proyecto7', db='proyecto7')
    cur = miConexion.cursor()
    cur.execute(f'insert into cardgame (cardgame_id,players,start_time,rounds,deck_id,end_time) values({cardgame["cardgame_id"]},{cardgame["players"]},"{cardgame["start_hour"]}",{cardgame["rounds"]},"{cardgame["deck_id"]}","{cardgame["end_hour"]}");')
    miConexion.commit()
    miConexion.close()
...
def generategame_id():
    ...
    miConexion.close()
    return a
def implementplayergame():
    for i in player_game[gaame]:
        miConexion = mysql.connector.connect(host='sieteymedio.mysql.database.azure.com', user='marcelogr04',
                                             passwd='Proyecto7', db='proyecto7')
        cur = miConexion.cursor()
        cur.execute(f'insert into game (game_id,player_id,cardgame_id,initial_card_id,start_points,end_points) values("{gaame}","{i}","{player_game[gaame][i]["cardgame_id"]}","{player_game[gaame][i]["initial_card_id"]}",{player_game[gaame][i]["starting_points"]},{player_game[gaame][i]["ending_points"]});')
        miConexion.commit()
    miConexion.close()
def implementround():
    for i in player_game_round:
        for j in player_game_round[i]:
            miConexion = mysql.connector.connect(host='sieteymedio.mysql.database.azure.com', user='marcelogr04',
                                             passwd='Proyecto7', db='proyecto7')
            cur = miConexion.cursor()
            cur.execute(f'insert into round (round_num,player_id,cardgame_id,start_round_points,end_round_points,is_bank,bet_points,cards_value,game_id) values({i},"{j}",{player_game_round[i][j]["cardgame_id"]},{player_game_round[i][j]["starting_round_points"]},{player_game_round[i][j]["endind_round_points"]},{player_game_round[i][j]["is_bank"]},{player_game_round[i][j]["bet_points"]},{player_game_round[i][j]["cards_value"]},{player_game_round[i][j]["game_id"]});')
            miConexion.commit()
    miConexion.close()
def more_earnings():
    miConexion = mysql.connector.connect(host='sieteymedio.mysql.database.azure.com', user='marcelogr04',
                                         passwd='Proyecto7', db='proyecto7')
    cur = miConexion.cursor()
    cur.execute("SET  sql_mode = 'STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION'")
    cur.execute("select count(ganadas), player_name, player_id from wins_pycharm group by player_id order by count(ganadas)desc;")
    cont=0
    for ganadas, player_name, player_id in cur.fetchall():
        cont=cont+1
        print("-"*30)
        print(f"Position {cont}")
        print("Name:",str(player_name).rjust(24))
        print("Wins:",str(ganadas).rjust(24))
        print("-" * 30)
    miConexion.close()
    input("Press any key to continue:")
    return ranking()
...
def more_games():
    ...
    cur = miConexion.cursor()
    cur.execute("SET  sql_mode = 'STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION'")
    cur.execute("select player_name, jugadas from plays_game;")
    cont = 0
    for player_name, jugadas in cur.fetchall():
        cont = cont + 1
        print("-" * 30)
        print(f"Position {cont}")
        print("Name:", str(player_name).rjust(24))
        print("Plays:", str(jugadas).rjust(23))
        print("-" * 30)
    miConexion.close()
    input("Press any key to continue:")
    return ranking()
def more_minutes():
    ...
    cur = miConexion.cursor()
    cur.execute("SET  sql_mode = 'STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION'")
    cur.execute("select player_name,minutos from see_minutes order by minutos desc;;")
    cont = 0
    for player_name, minutos in cur.fetchall():
        cont = cont + 1
        print("-" * 30)
        print(f"Position {cont}")
        print("Name:", str(player_name).rjust(24))
        print("Minutes:", str(minutos).rjust(21))
        print("-" * 30)
    miConexion.close()
    input("Press any key to continue:")
    return ranking()
def see_your_stats(i):
    miConexion = mysql.connector.connect(host='sieteymedio.mysql.database.azure.com', user='marcelogr04',
                                         passwd='Proyecto7', db='proyecto7')
    cur = miConexion.cursor()
    cur.execute("SET  sql_mode = 'STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION'")
    cur.execute(f'select minutos from see_minutes where player_id="{i}";')
    for minutos in cur.fetchall():
        m=minutos[0]
    cur.execute(f'select jugadas from plays_game where player_id="{i}"')
    for jugadas in cur.fetchall():
        p=jugadas[0]
    cur.execute(f'select count(ganadas) from wins_pycharm where player_id="{i}"')
    for ganadas in cur.fetchall():
        w=ganadas[0]
    print("*"*20,players[i]["name"],"*"*20)
    print("Wins:",str(w))
    print("Games played:",str(p))
    print("Minutes played:",str(m))
    miConexion.close()
def see_all_stats():
    for i in game:
        miConexion = mysql.connector.connect(host='sieteymedio.mysql.database.azure.com', user='marcelogr04',
